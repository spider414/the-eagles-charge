...
const ScumlRegistration = () => {
  ...
  const loadPreviousRequests = async () => {
    ...
    try {
      ...
      if (data) {
        setPreviousRequests(data.map((t: any) => ({ id: t.id, business_name: (t.api_response as any)?.business_name, status: t.status, certificate_number: (t.api_response as any)?.certificate_number, created_at: t.created_at, api_response: t.api_response })));
      }
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      await supabase.from("transactions").insert({ user_id: user!.id, transaction_type: "verification" as any, amount: PRICE, status: "processing" as any, data_plan: "scuml-registration", api_response: { business_name: businessName, rc_number: rcNumber, business_address: businessAddress, business_activity: businessActivity, contact_name: contactName, contact_phone: contactPhone, contact_email: contactEmail, additional_info: additionalInfo }, phone_number: contactPhone, balance_before: paymentData.balance_before, balance_after: paymentData.balance_after, description: "SCUML Registration" });
  ...
  const features = [
    { icon: "🛡️", title: "Anti-Money Laundering", desc: "Mandatory SCUML compliance certificate" },
    { icon: "📋", title: "DNFBPs Registration", desc: "For designated non-financial businesses" },
    { icon: "✅", title: "CBN/NFIU Compliance", desc: "Meet regulatory requirements" },
  ];
          ...
          <div className="container flex h-14 items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                ...
                <span className="text-base font-bold tracking-tight">SCUML Registration</span>
              </div>
            </div>
        ...
        <div className="flex flex-col lg:flex-row min-h-[calc(100vh-6rem)]">
          ...
          <div className="hidden lg:flex lg:w-[45%] gradient-hero text-primary-foreground relative overflow-hidden">
            ...
            <div className="relative z-10 flex flex-col justify-center p-12 space-y-8">
              <div className="space-y-4">
                ...
                <h2 className="text-3xl font-bold leading-tight">SCUML<br />Registration</h2>
                <p className="text-primary-foreground/70 text-sm leading-relaxed max-w-md">Special Control Unit against Money Laundering — mandatory compliance certificate for Designated Non-Financial Businesses and Professions.</p>
              </div>
              ...
              <div className="flex items-start gap-3 bg-primary-foreground/5 backdrop-blur-sm rounded-xl p-4 border border-primary-foreground/10">
                ...
                <div className="text-xs text-primary-foreground/70">
                  <p className="font-semibold text-primary-foreground text-sm">Processing: 5-10 business days</p>
                  <p className="mt-1">Your SCUML certificate number will be available once completed. Fee: ₦{PRICE.toLocaleString()}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="flex-1 lg:w-[55%]">
            ...
            <div className="lg:hidden gradient-hero text-primary-foreground p-6 space-y-3">
              <h2 className="text-xl font-bold">SCUML Registration</h2>
              <p className="text-xs text-primary-foreground/70">Anti-Money Laundering compliance certificate. Processing takes 5-10 business days. Fee: ₦{PRICE.toLocaleString()}</p>
            </div>
              ...
              {activeTab === "register" && (
                <div className="space-y-5 animate-fade-in">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">Provide your business details for SCUML registration</p>
                  </div>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="space-y-3">
                      <div className="space-y-1.5"><Label className="text-xs font-medium">Business Name <span className="text-destructive">*</span></Label><Input placeholder="Registered business name" value={businessName} onChange={(e) => setBusinessName(e.target.value)} className="h-11" /></div>
                      <div className="space-y-1.5"><Label className="text-xs font-medium">RC/BN Number</Label><Input placeholder="CAC registration number" value={rcNumber} onChange={(e) => setRcNumber(e.target.value)} className="h-11" /></div>
                      <div className="space-y-1.5"><Label className="text-xs font-medium">Business Address</Label><Input placeholder="Registered address" value={businessAddress} onChange={(e) => setBusinessAddress(e.target.value)} className="h-11" /></div>
                      <div className="space-y-1.5"><Label className="text-xs font-medium">Nature of Business</Label><Input placeholder="e.g. Financial services, Real estate" value={businessActivity} onChange={(e) => setBusinessActivity(e.target.value)} className="h-11" /></div>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5"><Label className="text-xs font-medium">Additional Information</Label><Textarea placeholder="Any additional details..." value={additionalInfo} onChange={(e) => setAdditionalInfo(e.target.value)} rows={3} /></div>
                  ...
                  <div className="bg-muted/50 rounded-xl p-4 space-y-4">
                    ...
                    <Button className="w-full h-12 text-base font-bold" onClick={handleSubmit} disabled={isSubmitting}>
                      {isSubmitting ? <><Clock className="h-4 w-4 mr-2 animate-spin" />Submitting...</> : <>Submit Registration <ChevronRight className="h-4 w-4 ml-1" /> — ₦{PRICE.toLocaleString()}</>}
                    </Button>
                  </div>
                </div>
              )}
              ...
              {activeTab === "retrieve" && (
                <div className="space-y-4 animate-fade-in">
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground">View and track your SCUML registration requests</p>
                  </div>
            ...
            <div className="border-t border-border p-4 text-center text-[10px] text-muted-foreground">
              © {new Date().getFullYear()} The Eagles Charge • SCUML Registration Service
            </div>
          </div>
        </div>
      </div>
